...
public class QueryParserBaseTests {
	...
	public void testNamedObjectsGroupQuery() {
		...
		GroupQuery groupQueryMock = Mockito.mock(GroupQuery.class);

		Mockito.when(groupQueryMock.getGroupByFunctions()).thenReturn(functionList);
		Mockito.when(groupQueryMock.getGroupByQueries()).thenReturn(queriesList);
		
		NamedObjectsGroupQuery decorator = new NamedObjectsGroupQuery(groupQueryMock);
		...
		Map<Object, String> objectNames = decorator.getNamesAssociation();
		
		Assert.assertEquals("nameFunc0", objectNames.get(functionList.get(0)));
		Assert.assertEquals("nameFunc1", objectNames.get(functionList.get(1)));
		Assert.assertEquals("nameQuery0", objectNames.get(queriesList.get(0)));
		Assert.assertEquals("nameQuery1", objectNames.get(queriesList.get(1)));
	}
	...
	public void testNamedObjectsGroupQueryForAnInexistentGroupObject() {
		GroupQuery groupQueryMock = Mockito.mock(GroupQuery.class);
		NamedObjectsGroupQuery decorator = new NamedObjectsGroupQuery(groupQueryMock);
		decorator.setName(Mockito.mock(Query.class), "some name");
	}
