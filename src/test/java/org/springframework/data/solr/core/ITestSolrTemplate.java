...
public class ITestSolrTemplate extends AbstractITestWithEmbeddedSolrServer {
	...
	public void testRegularGroupQuery() {
		...
		SimpleGroupQuery groupQuery = new SimpleGroupQuery(new SimpleStringCriteria("*:*"));
		groupQuery.addSort(new Sort("name","id"));
		groupQuery.addGroupByField("name");
		groupQuery.addGroupByFunction(f);
		groupQuery.addGroupByQuery(q1);
		groupQuery.addGroupByQuery(q2);
		groupQuery.setGroupLimit(2);

		// asserts result page
		GroupResultPage<ExampleSolrBean> groupResultPage = solrTemplate.queryForGroupPage(groupQuery, ExampleSolrBean.class);
